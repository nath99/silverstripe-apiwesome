...
namespace nglasl\APIwesome\Services;
...
use nglasl\APIwesome\Controllers\APIwesome;
...
class APIwesomeService
{
    ...
    public function retrieveValidated($class, $limit = null, $sort = null, $filters = null)
    {
        ...
        if (in_array($class, array_map('strtolower', ClassInfo::subclassesFor(DataObject::class))) && ($configuration = DataObjectOutputConfiguration::get_one(DataObjectOutputConfiguration::class, array(
            'LOWER(IsFor) = ?' => $class
        ))) && ($temporaryClass = DataObject::get_one($class))) {
            $class = DataObject::getSchema()->baseDataClass($temporaryClass->ClassName);
            ...
            foreach (ClassInfo::subclassesFor($class) as $subclass) {
                ...
                $subclassFields = DataObject::getSchema()->databaseFields($subclass, true);
                if (ClassInfo::hasTable($subclass)) {

                    // Determine the versioned table.

                    $same = ($subclass === $class);
                    if ($subclass::has_extension(Versioned::class)) {
    ...
    public function retrieveJSON($objects, $filters = null, $attributeVisibility = false, $contentHeader = false, $callback = false)
    {
        ...
        foreach ($objects as $object) {
            ...
            $class = $classExists ? $object['ClassName'] : DataObject::class;
        ...
        $header = false;
        $class = is_subclass_of($objects[0]['ClassName'], SiteTree::class) ? SiteTree::class : (is_subclass_of($objects[0]['ClassName'], File::class) ? File::class : $objects[0]['ClassName']);
        if ($callback && ($configuration = DataObjectOutputConfiguration::get_one(DataObjectOutputConfiguration::class, array(
            'IsFor = ?' => $class
        )))) {
                ...
                if (($attribute !== 'ClassName') && ($attribute !== 'RecordClassName')) {
                    ...
                    if ($relationship && ($relationObject = DataObject::get_by_id($object['ClassName'], $object['ID'])) && $relationObject->hasMethod($relationship) && ($value != 0)) {
                        ...
                        if ($attributeVisibility) {
                            ...
                            $class = is_subclass_of($relationObject->ClassName, SiteTree::class) ? SiteTree::class : (is_subclass_of($relationObject->ClassName, File::class) ? File::class : $relationObject->ClassName);
                            $relationConfiguration = DataObjectOutputConfiguration::get_one(DataObjectOutputConfiguration::class, array(
                                'IsFor = ?' => $class
                            ));
                            ...
                            foreach (ClassInfo::subclassesFor($class) as $subclass) {
                                ...
                                $subclassColumns = array();
                                foreach (DataObject::getSchema()->databaseFields($subclass, true) as $column => $type) {
    ...
    public function retrieveStaged($page, $output)
    {
        ...
        if ((($stage === 'Stage') || ($stage === 'Live')) && (($output === 'JSON') || ($output === 'XML'))) {
            ...
            $temporary = array(
                0 => SiteTree::get_by_id(SiteTree::class, $page)->toMap()
            );
    ...
    public function parseJSON($JSON)
    {
        ...
        $temporary = json_decode($JSON, true);
        return isset($temporary[APIwesome::class]['DataObjects']) ?
            $temporary[APIwesome::class]['DataObjects'] :
            null;
    }

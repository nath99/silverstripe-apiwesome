...
class DataObjectOutputConfiguration extends DataObject
{
    ...
    private static $exclusions = array(
        APIwesomeToken::class,
        DataObjectOutputConfiguration::class,
        Group::class,
        LoginAttempt::class,
        Member::class,
    ...
    public static function customise_data_objects($type, $objects = null)
    {
        ...
        if (is_array($objects) && (strtolower($type) === 'exclude')) {
            ...
            CMSMenu::remove_menu_item(APIwesomeAdmin::class);
        }
    }
    ...
    public function requireDefaultRecords()
    {
        ...
        foreach (DB::table_list() as $table) {
            ...
            if (!class_exists($table)) {
                $existing = DataObjectOutputConfiguration::get_one(DataObjectOutputConfiguration::class, array(
                    'IsFor = ?' => $table
                ));
        ...
        foreach ($objects as $object) {
            ...
            if ($existing && (self::$disabled || (get_parent_class($object) !== DataObject::class) || ClassInfo::classImplements($object, TestOnly::class) || ((count($inclusions) > 0) && !in_array($object, $inclusions)) || ((count($inclusions) === 0) && in_array($object, $exclusions)))) {
            ...
            } elseif (!$existing && !self::$disabled && (get_parent_class($object) === DataObject::class) && !ClassInfo::classImplements($object, TestOnly::class) && (((count($inclusions) > 0) && in_array($object, $inclusions)) || ((count($inclusions) === 0) && !in_array($object, $exclusions)))) {
    ...
    public function getCMSFields()
    {
        ...
        if (DataObject::get_one($this->IsFor)) {
            ...
            $class = is_subclass_of($this->IsFor, SiteTree::class) ? SiteTree::class : (is_subclass_of($this->IsFor, File::class) ? File::class : $this->IsFor);
            $columns = array();
